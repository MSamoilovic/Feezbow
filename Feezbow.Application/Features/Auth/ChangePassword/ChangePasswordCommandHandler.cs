...
) : IRequestHandler<ChangePasswordCommand, ChangePasswordCommandResponse>
{
    ...
    )
    {
        ...
        {
            logger.LogWarning(
                "Change password failed: incorrect old password for user {UserId}",
                user.Id
            );
            return new ChangePasswordCommandResponse(
                Result<long>.Fail(user.Id, "Old Password is incorrect")
            );
        }
        ...
        return new ChangePasswordCommandResponse(
            Result<long>.Success(user.Id, "Password updated successfully")
        );
    }
}

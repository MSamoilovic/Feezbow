...
using Feezbow.Application.Features.ShoppingLists.CleanShoppingList;
...
public class ShoppingListsController(IMediator mediator) : ControllerBase
{
    ...
    )
    {
        var response = await mediator.Send(
            new GetShoppingListsByProjectQuery(projectId, includeArchived),
            cancellationToken
        );
        return Ok(response);
    }
    ...
    public async Task<IActionResult> CreateList(
        long projectId,
        [FromBody] CreateShoppingListCommand command,
        CancellationToken cancellationToken
    )
    {
        var response = await mediator.Send(
    ...
    public async Task<IActionResult> RenameList(
        long projectId,
        long listId,
        [FromBody] RenameShoppingListCommand command,
        CancellationToken cancellationToken
    )
    {
        var response = await mediator.Send(
    ...
    {
        var response = await mediator.Send(
            new ArchiveShoppingListCommand(listId),
            cancellationToken
        );
        return Ok(response);
    }
    ...
    {
        var response = await mediator.Send(
            new DeleteShoppingListCommand(listId),
            cancellationToken
        );
        return Ok(response);
    }
    ...
    public async Task<IActionResult> AddItem(
        long projectId,
        long listId,
        [FromBody] AddShoppingListItemCommand command,
        CancellationToken cancellationToken
    )
    {
        var response = await mediator.Send(
    ...
    public async Task<IActionResult> UpdateItem(
        ...
        long listId,
        long itemId,
        [FromBody] UpdateShoppingListItemCommand command,
        CancellationToken cancellationToken
    )
    {
        var response = await mediator.Send(
    ...
    {
        var response = await mediator.Send(
            new RemoveShoppingListItemCommand(listId, itemId),
            cancellationToken
        );
        return Ok(response);
    }
    ...
    /// <summary>
    /// Returns an AI-generated cleanup proposal for the shopping list: duplicate merges and
    /// name corrections. Purchased items are excluded. This is a proposal only — nothing is
    /// changed until the frontend applies the suggested merges and renames via existing item
    /// update/remove endpoints.
    /// </summary>
    [HttpPost("{listId:long}/clean")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    ...
    {
        return Ok(await mediator.Send(new CleanShoppingListQuery(listId), cancellationToken));
    }

    [HttpPatch("{listId:long}/items/{itemId:long}/purchased")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> ToggleItemPurchased(
        long projectId,
        long listId,
        long itemId,
        [FromBody] ToggleItemPurchasedCommand command,
        CancellationToken cancellationToken
    )
    {
        var response = await mediator.Send(
